...
class RegisterViewController: DiaryBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        email.subscribe(onNext: { (state) in
            self.registerView.codeButton.backgroundColor = state && self.registerView.codeButton.isEnabled == true ? cz_HexadecimalColor(hexadecimal: AppContentColor) : cz_RgbColor(239, 240, 244, 1)
        ...
        registerView.codeButton.rx.tap.subscribe(onNext: { () in
            ...
            self.viewModel.sendVerificationCode(to: self.registerView.emailTextField.text!).subscribe(onSuccess: { (model) in
                ...
                DispatchQueue.main.async(execute: {
                    self.registerView.codeButton.backgroundColor = cz_RgbColor(239, 240, 244, 1)
        ...
        NotificationCenter.default.rx.notification(Notification.Name(rawValue: "RegistrationVerificationCode"))
                                    ...
                                    .subscribe(onNext: { (notification) in
                                        if (notification.object! as! Int) == 0 {
                                            DispatchQueue.main.async(execute: {
                                                self.registerView.codeButton.isEnabled = true
                                                self.registerView.codeButton.backgroundColor = cz_HexadecimalColor(hexadecimal: AppContentColor)
        ...
        zip.subscribe(onNext: { (state) in
                self.registerView.registerButton.backgroundColor = state == true ? cz_HexadecimalColor(hexadecimal: AppContentColor) : cz_RgbColor(239, 240, 244, 1)
            }).disposed(by: rx.disposeBag)
